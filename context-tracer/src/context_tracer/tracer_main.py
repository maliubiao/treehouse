...
from .tracer import TraceConfig, color_wrap, start_trace, stop_trace
...
def print_debug_summary(report_path: Path) -> None:
    ...
    print(color_wrap("  ↘ B-CALL   - 边界调用 (从追踪文件到非追踪文件)", "call"))
    print(color_wrap("  ↗ B-RETURN - 边界返回", "return"))
    print(color_wrap("  ⚠ B-EXCEPTION - 边界异常", "error"))
    print(color_wrap("  ↘ C-CALL   - C函数调用 (Python 3.12+)", "trace"))
    print(color_wrap("  ↗ C-RETURN - C函数返回 (Python 3.12+)", "trace"))
    print(color_wrap("  Δ VARIABLES - 变量创建/修改/删除", "var"))
    print(color_wrap("  ▷ LINE     - 执行的源代码行", "line"))
    print(color_wrap("  ⚠ EXCEPTION - 在追踪的函数内发生的异常", "error"))
    print(color_wrap("\n调试功能说明:", "line"))
