...
from context_tracer.container import DataContainerReader
...
class TestTruncateReprValue(unittest.TestCase):
    ...
    def test_recursion_limit_in_truncate_repr(self):
...
class TestTraceConfig(BaseTracerTest):
    ...
    def test_initialization_defaults(self):
        ...
        # New assertion for container
        self.assertFalse(config.enable_container)
    ...
    def test_container_config(self):
        # Test error if container enabled without key
        with self.assertRaisesRegex(ValueError, "container_key.*is required"):
            TraceConfig(enable_container=True)

        # Test valid key
        key_hex = "00" * 16
        config = TraceConfig(enable_container=True, container_key=key_hex)
        self.assertTrue(config.enable_container)
        self.assertEqual(config.container_key_bytes, bytes.fromhex(key_hex))

        # Test invalid key length
        with self.assertRaisesRegex(ValueError, "key must be 16, 24, or 32 bytes"):
            TraceConfig(enable_container=True, container_key="00" * 10)
    ...
    def test_from_yaml(self):
        ...
        self.assertTrue(config.trace_c_calls)  # New assertion
        self.assertTrue(config.enable_container)
        self.assertEqual(config.container_key_bytes, bytes.fromhex(sample_config["container_key"]))
...
class TestCallTreeHtmlRender(BaseTracerTest):
    ...
    def setUp(self):
        super().setUp()
        # Test decoupling by initializing with config instead of logic
        self.config = TraceConfig(report_name="test_report.html")
        self.renderer = CallTreeHtmlRender(config=self.config)
...
class TestIntegration(BaseTracerTest):
    ...
    def test_e2e_container_generation(self):
        """
        E2E test to verify that the data container is created correctly
        when the corresponding CLI flags are used.
        """
        script_content = dedent("""
            def main():
            ...
            if __name__ == "__main__":
                main()
        """)
        script_path = self.test_dir / "container_script.py"
        script_path.write_text(script_content, encoding="utf-8")

        key_hex = "0123456789abcdef0123456789abcdef"
        container_path = self.test_dir / "test_data.bin"
        cli_args = [
            "--enable-container",
            "--container-key",
            key_hex,
            "--container-path",
            str(container_path),
            "--disable-html",  # Disable HTML to speed up test
        ]
        process = self._run_e2e_test(script_path, cli_args)

        self.assertEqual(process.returncode, 0, f"Tracer process exited with error. Stderr: {process.stderr}")
        self.assertTrue(container_path.exists(), f"Container file not found at {container_path}")

        # Verify the container content
        key_bytes = bytes.fromhex(key_hex)
        reader = DataContainerReader(container_path, key_bytes)
        reader.open()
        events = list(reader)
        reader.close()
