...
class Tracer:
    ...
    def on_step_hit(self, frame):
        ...
        if line_entry.IsValid():
            source_info = "%s:%d" % (line_entry.GetFileSpec().GetFilename(), line_entry.GetLine())

        # Log detailed step information
        self.logger.info("Step hit at %s (0x%x) module: %s", function, pc, frame.module)
        self.logger.info("0x%x %s %s ; %s", pc, mnemonic, operands, function)
        if source_info:
            self.logger.info("  Source location: %s", source_info)
    ...
    def dump_modules_info(self):
        ...
        for module_info in self._module_ranges.values():
            ...
            # Display the module panel
            console.print(Panel.fit(module_table, border_style="blue", padding=(0, 1)))
            console.print()  # Add spacing between modules

    def find_module_by_address(self, address):
        """Find module and section containing the given address.
        ...
        for module_name, module_info in self._module_ranges.items():
            for section_info in module_info["sections"]:
                if section_info["start_addr"] <= address < section_info["end_addr"]:
                    offset = address - section_info["start_addr"]
                    return (module_info["module"], section_info["section"], offset)
    ...
    def _handle_special_stop(self, thread, stop_reason):
        ...
        # Default action is to continue unless configured otherwise
        if self.config_manager.config.get("auto_continue", True):
            process.Continue()
...
def parse_args():
    ...
    parser.add_argument("--auto-continue", action="store_true", help="Auto continue after hit")
...
def main():
    ...
    config = {}
    if args.condition:
        config["breakpoint_condition"] = args.condition
    if args.auto_continue:
        config["auto_continue"] = args.auto_continue
    if args.verbose:
        tracer.logger.setLevel(logging.DEBUG)
        config.update({"log_target_info": True, "log_module_info": True, "log_breakpoint_details": True})
    tracer.config_manager.config.update(config)
    tracer.start()
