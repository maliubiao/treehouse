...
class CodeTracer:
    ...
    def _process_symbol(self, symbol_name: str, symbol: dict) -> str:
        ...
        symbol_range = symbol["block_range"]
        start_pos, end_pos = symbol_range
        for prev_start, prev_end in self.symbol_ranges:
            if start_pos <= prev_end and end_pos >= prev_start:
                print(f"Skipping overlapping symbol: {symbol_name}")
                return ""
        self.symbol_ranges.append(symbol["block_range"])
...
"""
    ...
    def _record_response(self, batch_id: str, response: str):
        ...
        transformed_map = dict(transformed_symbols)
            ...
            if batch["batch_id"] == batch_id:
                ...
                for symbol_snippet in batch.get("symbol_snippets", []):
                    ...
                    if original_code or transformed_code:
                        self.file_transformations[f"{self.file_path}/{symbol_name}"] = {
                            "file_path": self.file_path,
        ...
        self._save_file_transformations()
    ...
    def _save_file_transformations(self):
        """保存文件级符号转换关系到单独文件"""
        debug_dir = Path("trace_debug") / "file_transformations"
        debug_dir.mkdir(exist_ok=True)
        file_name = Path(self.file_path).name
        transform_file = debug_dir / f"{file_name}_transformations.json"

        try:
            with open(transform_file, "w", encoding="utf-8") as f:
                json.dump(self.file_transformations, f, indent=2, ensure_ascii=False)
        except (IOError, OSError) as e:
            print(f"Error saving file transformations: {str(e)}")
    ...
    @classmethod
    def get_file_transformations(cls, file_path: str) -> Dict:
        """获取指定文件的转换关系"""
        debug_dir = Path("trace_debug") / "file_transformations"
        ...
        try:
            ...
            print(f"Error loading file transformations: {str(e)}")
            return {}
